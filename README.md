...
* [`src/clj/tsca_webapp/`](src/clj/tsca_webapp/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/tsca_webapp/`](src/cljs/tsca_webapp/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/tsca_webapp/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `tsca-webapp` project root directory
...
in the [`src/clj/tsca_webapp/`](src/clj/tsca_webapp/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/tsca_webapp/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tsca-webapp.example
  (:require [tsca-webapp.config :as config])
